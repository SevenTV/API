...
func (r *avatars) Handler(ctx *rest.Ctx) errors.APIError {
	...
	if err != nil {
		logrus.WithError(err).Error("mongo, failed to spawn aggregation for user avatars")
		return errors.ErrInternalServerError().SetDetail(err.Error())
	}
	v := &aggregatedAvatarsResult{}
	cur.Next(ctx)
	if err = cur.Decode(v); err != nil {
		logrus.WithError(err).Error("mongo, failed to decode aggregated avatars data")
		return errors.ErrInternalServerError().SetDetail(err.Error())
	}
